        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="rounded-3xl bg-gradient-to-br from-indigo-700 to-sky-600 p-8 text-white shadow-xl">
                ...
                <div class="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <h1 class="text-3xl font-black sm:text-4xl">{{ auth()->check() ? 'Moje grupy' : 'Przegladaj grupy' }}</h1>
                        <p class="mt-3 max-w-2xl text-sm leading-6 text-indigo-50">
                            Nazwa grupy musi byc unikalna dla jednego wlasciciela, ale rozni uzytkownicy moga uzyc tej samej nazwy.
                        </p>
                    </div>
            ...
            <div class="grid gap-6 {{ auth()->check() ? 'lg:grid-cols-[0.9fr_1.1fr]' : '' }}">
                @auth
                ...
                @else
                    <div class="rounded-2xl border border-indigo-200 bg-indigo-50 p-6 shadow-sm dark:border-indigo-900 dark:bg-indigo-950">
                        <h2 class="text-lg font-black text-indigo-950 dark:text-indigo-100">Chcesz utworzyc wlasna grupe?</h2>
                        <p class="mt-3 text-sm leading-6 text-indigo-900 dark:text-indigo-200">
                            Przegladanie jest publiczne, ale dodawanie grup i wydatkow wymaga konta.
                        </p>
                        <div class="mt-5 flex flex-wrap gap-3">
                            <a href="{{ route('login') }}" class="rounded-xl bg-indigo-600 px-5 py-3 text-sm font-black text-white hover:bg-indigo-700">Zaloguj sie</a>
                            @if(Route::has('register'))
                                <a href="{{ route('register') }}" class="rounded-xl border border-indigo-300 px-5 py-3 text-sm font-black text-indigo-800 hover:border-indigo-500 dark:text-indigo-100">Utworz konto</a>
                            @endif
                        </div>
                    </div>
                @endauth
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-gray-900">
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ $isAdmin ? 4 : 3 }}" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
